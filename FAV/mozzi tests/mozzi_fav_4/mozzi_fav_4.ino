...
void setup(){
  ...
  aDel.setFeedbackLevel(100); // can be -128 to 127
}
...
int mapStatesB(byte state) {
  ...
  return map(int(state), 0, 2, 700, 1023);
}
...
int updateAudio(){
  ...
  return w/8 + aDel.next(w, deltime); // mix some straight signal with the delayed signal
