...
void setup(){
 ...
 // kDelSamps.setFreq(.163f); // set the delay time modulation frequency (ie. the sweep frequency)
  kDelSamps.setFreq(.163f/4); // set the delay time modulation frequency (ie. the sweep frequency)
...
int mapStatesA(byte state) {
  if (state0 == 0 && state1 == 0 && state2 != 0) {
    ...
    volume = volume - 0.05;
    volume = constrain(volume, 0, VOLUME_MAX);
...
int mapStatesB(byte state) {
  ...
  return map(int(state), 0, 2, 200, 300);

}

int mapStatesC(byte state) {  
  state = constrain(state, 1, 2);
  return map(int(state), 0, 2, 400, 500);
}
...
void updateControl(){
  ...
  aTriangle.setFreq(mapStatesA(state0));
